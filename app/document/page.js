...
function App() {
  ...
  const validFormats = {
  validImages :['png', 'jpeg', 'svg', 'ico', 'gif', 'psd', 'webp', 'bmp', 'jpg', 'tiff', 'jpeg', 'tga', 'ico', 'eps'],
  validPdf : ['pdf'],
  ...
  image_svg:['png', 'jpeg',  'ico', 'gif', 'psd', 'webp' ,'bmp', 'jpg', 'tiff', 'jpeg', 'tga', 'ico', 'eps','pdf'],
  svg:['svg'],
...
const handleFileUpload = async (e) => {
  try {
    if (files.length === 0) {
      alert('Please select one or more Document files first.');
      return;
    }
    ...
    await Promise.all(files.map(async (file, index) => {
      ...
      if (validFormats.validImages.includes(fileExtension)) {
        ...
        if (validFormats.validVidoAudio.includes(formatChose)) {
          uploadUrl = `${apiUrl}/DocumentAudio`;
  ...
  useEffect(() => {
    ...
    const checkConversionProgress = async () => {
      try {
        ...
        const responseDocumentAudio = await axios.get(`${apiUrl}/progressDocumentAudio`);
        const progress0 = responseDocumentAudio.data.progress;
...
{   
  category:[{la:"Image"},{la:"Document"},],
  Document: ['CSV', 'XML', 'PDF', 'ODT', 'RTF', 'TEXT', 'TXT'],
  Image: ['PNG', 'JPEG','GIF',],
  ...
  fileAvailableFormats = [
    {   
      ...
      Document: ['PDF', 'TXT', 'DOCX', 'DOC', 'WORD', 'XLSX', 'XLS', 'EXCEL', 'HTML', 'PPT', 'PPTX'],
      Image: ['PNG', 'JPEG','GIF',],
  ...
  fileAvailableFormats = [
    ...
    {   
      category:[{la:"Audio"},{la:"Document"},{la:'Image'}],
      Audio: ['MP3', 'AIFF', 'AAC', 'WMA', 'FLAC', 'M4A', 'MP2', 'OGA'],
Document: ['MOV', 'MP4', 'FLV', 'MKV', 'AVI', 'WEBM', 'WMV',  'OGV', 'MPG', 'MPEG', 'M4V', 'MJPEG', 'HEVC', 'SWF', 'TS', 'VOB'],
Image:['GIF'],
...
<div className="convert" onDrop={handleDrop}onDragOver={handleDragOver}>
      ...
      <h1 className='title'>Sit a Document and Let us convert it</h1>
      <p className='description'>Seamlessly switch Document formats with our online converter.</p>
...
<p className='update'>"      <BsFillLockFill style={{color:"#2ecc71"}} /> Drop your Documents here"</p>
...
<div className='describe_how_convert'>
  <div className='full_how_convert'>
    ...
    <h2 className='how_convert'>How to convert a Document ?</h2>
    </div>
  <p className='description_p'>1.Convert your Documents easily by starting with selecting them using the 'Choose Documents' button</p>
  <p className='description_p'>2.Initiate the conversion process by choosing the desired format and clicking convert to</p>
  <p className='description_p'>3.Once the conversion is complete, click 'Download' to retrieve your converted Documents</p>
</div>
...
<div className='how_work_card'>
  ...
  Just upload your Documents and tap 'Convert'. Our tool guarantees the highest quality conversion. Unbeatable Features
  </p>
</div>
...
<div className='how_work_card'>
  ...
  <p className='how_work_description'>
  Effortlessly convert batches Documents with our tool, which accommodates any Document formats.
  </p>
</div>
...
<div className='how_work_card'>
  ...
  <p className='how_work_description'>
  Enjoy the benefits of a free, secure, and universally compatible tool accessible from any web browser. For added security and privacy, Documents are automatically deleted after a few hours.
  </p>
</div>
...
<title>Document Converter Tool: Effortless Format Conversion & Optimization</title>
  <meta name="description" content="Discover the ultimate solution for converting Documents with our versatile Image Converter Tool. Easily switch between formats like JPEG, PNG, GIF, and more to suit your needs. Streamline your workflow by resizing, compressing, or optimizing images for maximum efficiency. With our user-friendly interface, you can transform your visuals in seconds, ensuring compatibility across platforms. Say goodbye to format headaches and hello to seamless image conversion with our powerful tool." />
